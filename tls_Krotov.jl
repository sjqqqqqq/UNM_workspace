...
problem = ControlProblem(
    ...
    prop_method=ExpProp,
    pulse_options=Dict(
        ϵ => Dict(:lambda_a => 1.0, :update_shape => (t->1.0))
    ),
    J_T=J_T_sm,
    check_convergence=res -> begin
        ((res.J_T < 1e-04) && (res.converged = true) && (res.message = "J_T < 10⁻⁴"))
    end,
);
...
opt_result_Krotov = @optimize_or_load(
    datadir("TLS", "Krotov_opt_result.jld2"),
    problem;
    method=Krotov,
    prop_method=ExpProp,
);

fig = plot_control(opt_result_Krotov.optimized_controls[1], tlist)  # This is test